...
PangolinDSOViewer::PangolinDSOViewer(int w, int h, ros::NodeHandle &n, bool startRunThread)
{
  ...
  pub_cloud = n.advertise<sensor_msgs::PointCloud2>("point_cloud", 1000);
...
void PangolinDSOViewer::run()
{
  ...
  while( running && ros::ok() )
    ...
    if(setting_render_display3D && keyframes.size() != 0)
      ...
      sensor_msgs::PointCloud2 output;  //PointCloud转PointCloud2
      if (!sensor_msgs::convertPointCloudToPointCloud2 (PC_all, output))
      {
