...
std::vector<Eigen::Vector4f> KeyFrameDisplay::getPoint(bool canRefresh, float scaledTH, float absTH, int mode, float minBS, int sparsity)
{
  ...
  Eigen::Matrix4f Tcw = camToWorld.matrix().cast<float>();
  ...
  for(int i=0; i<numSparsePoints; i++)
  {
    ...
    for(int pnt=0; pnt<patternNum; pnt++)
    {
      ...
      p(0) = ((originalInputSparse[i].u+dx)*fxi + cxi) * depth;
      p(1) = ((originalInputSparse[i].v+dy)*fyi + cyi) * depth;
      p(2) = depth*(1 + 2*fxi * (rand()/(float)RAND_MAX-0.5f));
      p(3) = 1;
